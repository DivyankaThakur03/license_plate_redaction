...
def tiler_sink_pad_buffer_probe(pad, info, u_data):
    ...
    while l_frame is not None:
        ...
        while l_obj is not None:
            ...
            lp = n_frame[top:top+height, left:left+width]
            roi = cv2.GaussianBlur(lp,(25,25),7)
            n_frame[top:top+height, left:left+width] = roi

            # frame =  n_frame[top:top+height, left:left+width]
            lp1 = n_frame[top:top+height, left:left+width]
            cv2.imwrite("frame.jpg", lp1)
            # print("roi",roi)

            result = ocr.ocr("frame.jpg")
            print(result)
...
def main(args):
    ...
    pgie.set_property('config-file-path', "/opt/nvidia/deepstream/deepstream-6.1/sources/deepstream_python_apps/apps/ds_lp_redaction/config_infer_primary_yoloV5.txt")
